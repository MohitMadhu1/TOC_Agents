...
import { Share2, Zap, Terminal, GitBranch, ArrowRight, ChevronLeft, ChevronRight, Play, Swords, Layers } from 'lucide-react';

const modules = [
  { id: '2.1', title: 'Regex Playground', icon: <Play size={32}/>, color: '#00FFCC', desc: 'Standard operator engine with real-time matcher and AI-driven simplification.' },
  { id: '2.2', title: 'RE-FA Translator', icon: <GitBranch size={32}/>, color: '#00E5FF', desc: 'Thompson Construction and State Elimination lab. Convert machines to code.' },
  { id: '2.3', title: 'Pumping Duel', icon: <Swords size={32}/>, color: '#BDFF00', desc: 'Gamified strategy mode. Prove non-regularity in a turn-based duel vs the CPU.' },
];

const Phase2Dashboard: React.FC<{ 
    onBack: () => void, 
    onLaunchRegex: () => void, 
    onLaunchTranslator: () => void, 
    onLaunchDuel: () => void,
}> = ({ onBack, onLaunchRegex, onLaunchTranslator, onLaunchDuel }) => {
  const [activeIdx, setActiveIdx] = useState(0);
  const activeMod = modules[activeIdx];

  const handleLaunch = () => {
    if (activeMod.id === '2.1') onLaunchRegex();
    if (activeMod.id === '2.2') onLaunchTranslator();
    if (activeMod.id === '2.3') onLaunchDuel();
  };
